...
async def chat_with_ai(
    ...
    current_user: crud.get_current_user = Depends(get_current_user), # Assuming crud.get_current_user returns a User model
...
async def generate_health_alert(
    ...
    current_user: crud.get_current_user = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    try:
        genai.configure(api_key=os.getenv("GEMINI_API_KEY"))
        ...
        prompt = f"""Analyze the following user health data for a pregnant individual and provide a concise health alert or insight.
Focus on potential issues, recommendations, or interesting observations based on the provided data.
If there are no significant issues, provide a positive reinforcement or general tip.
Format the response as a JSON object with two fields: "title" (string, short title) and "description" (string, brief explanation or recommendation).

Current Week of Pregnancy: {health_data.current_week}
Recent Symptoms: {health_data.recent_symptom_logs}
Recent Weight Logs: {health_data.recent_weight_logs}
"""
