    ...
    if (!isMounted) {
      ...
      return (
         <aside
          ref={ref}
          className={cn(
            "fixed inset-y-0 left-0 z-20 hidden h-screen flex-col border-r bg-sidebar text-sidebar-foreground md:flex pb-14",
            "w-[var(--sidebar-width)]",
            className
          )}
          {...props}
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="footer"
      className={cn("mt-auto flex flex-col items-center gap-2 p-3", className)}
      {...props}
    />
  )
})
